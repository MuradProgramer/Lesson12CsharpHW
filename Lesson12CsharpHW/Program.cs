...
namespace Lesson12CsharpHW
{
    public class Program
    {
        ...
        static List<Debtor> debtors = new List<Debtor> {
            ...
            new Debtor("Paul H. Sturtz", DateTime.Parse("September 15, 1944"), "123-456-7890", "PaulHSturtz@dayrep.com", "759 Havanna Street Saxapahaw, NC 27340", 10437),
        ...
        static void Main(string[] args)
        {
            ...
            //List<Debtor> debtorsOrderedByGmail = debtors.FindAll(
            //        deb => deb.Email.Split('@')[1] == "rhyta.com" ||
            //        deb.Email.Split('@')[1] == "dayrep.com");

            //foreach (var item in debtorsOrderedByGmail)
            //{
            //    Console.Write("Name: {0,-25}", item.FullName);
            //    Console.WriteLine(" Ending Email: {0,-35}", item.Email.Split('@')[1]);
            //}
            ...
            //List<Debtor> debtorsOrderedByAge =
            //    debtors.FindAll(deb => DateTime.Now.Year - deb.BirthDay.Year >= 26 &&
            //                           DateTime.Now.Year - deb.BirthDay.Year < 36);

            //foreach (var item in debtorsOrderedByAge)
            //{
            //    Console.Write("Name: {0,-25}", item.FullName);
            //    Console.WriteLine(" Age: {0,-35}", DateTime.Now.Year - item.BirthDay.Year);
            //}

            /////////////////////////////////////////////////

            //// 3) Borcu 5000-den cox olmayan borclularic cixartmag
            //List<Debtor> debtorsOrderedByDebt = debtors.FindAll(deb => deb.Debt <= 5000);

            //foreach (var item in debtorsOrderedByDebt)
            //{
            //    Console.Write("Name: {0,-25}", item.FullName);
            //    Console.WriteLine(" Debt: {0,-35}", item.Debt);
            //}
            ...
            //List<Debtor> upgratedDebtor = debtors.FindAll(
            //        deb => deb.FullName.Length > 18 &&
            //        deb.Phone.ToList().FindAll(n => (int)n >= 50).Count() >= 7
            //);
            //foreach (var item in upgratedDebtor)
            //{
            //    Console.Write("Name: {0,-25}", item.FullName);
            //    Console.WriteLine(" Phone: {0,-35}", item.Phone);
            //}
            ...
            //List<Debtor> upgratedDebtors = debtors.FindAll(deb => deb.BirthDay.Month == 1);
            //foreach (var item in upgratedDebtors)
            //{
            //    Console.Write("Name: {0,-25}", item.FullName);
            //    Console.WriteLine(" Birthday: {0,-35}", item.BirthDay.ToShortDateString());
            //}

            /////////////////////////////////////////////////

            //// 6) Borcu, umumi borclarin orta borcunnan cox olan 
            ////    borclulari cixarmaq ve evvel familyaya gore sonra ise meblegin azalmagina gore sortirovka etmek

            //int cem = 0;
            //for (int i = 0; i < debtors.Count; i++)
            //    cem += debtors[i].Debt;
            //int ortaborc = cem / debtors.Count;
            //Console.WriteLine($"Orta borc: {ortaborc}\n\n");
            //List<Debtor> upgratedDebtors = debtors
            //    .FindAll(deb => deb.Debt > ortaborc)
            //    .OrderBy(deb => $"{deb.FullName.Split(' ')[1]} {deb.FullName.Split(' ')[2]}")
            //    .OrderByDescending(deb => deb.Debt)
            //    .ToList();
            //foreach (var item in upgratedDebtors)
            //{
            //    Console.Write("Name: {0,-25}", item.FullName);
            ...
            //List<Debtor> upgratedDebtors = debtors.FindAll(
            //        deb => !(deb.Phone.ToList().Contains('8'))
            //);
            //Console.WriteLine("  Surname     Age             Debt              Phone\n");
            //foreach (var item in upgratedDebtors)
            //{
            //    Console.Write("{0,-15}", $"{item.FullName.Split(' ')[1]} {item.FullName.Split(' ')[2]}");
            //    Console.Write("{0,-15}", DateTime.Now.Year - item.BirthDay.Year);
            //    Console.Write("{0,-15}", item.Debt);
            //    Console.WriteLine("{0,-15}", item.Phone);
            //}
            ...
            //List<Debtor> upgratedDebtors = debtors
            //    .FindAll(deb => deb.FullName.Any(c => deb.FullName.Split(c).Length > 3))
            //    .OrderBy(deb => deb.FullName)
            //    .ToList();
            ...
            //var list = debtors.OrderBy(deb => deb.Debt).ToList();
            //Console.WriteLine("    Name                 Debt\n");
            //for (int i = 0, k = list.Count - 1; i < 5; i++)
            //{
            //    Console.Write("{0, -25}", list[k].FullName);
            //    Console.WriteLine("{0, -20}", list[k--].Debt);
            //}
            ...
            //var result = debtors.Select(deb => deb.Debt).ToList();
            //int cem = 0;
            //foreach (var item in result)
            //    cem += item;
            //Console.WriteLine("All debts: {0}", cem);

            /////////////////////////////////////////////////

            //// 12) 2ci dunya muharibesin gormush borclularin siyahisi cixartmaq

            //List<Debtor> list = debtors
            //    .FindAll(deb => deb.BirthDay.Year < 1945);
            //Console.WriteLine("        Name              Year\n");
            //foreach (var item in list)
            //{
            //    Console.Write("{0,-25}", item.FullName);
            //    Console.WriteLine(" {0,-15}", item.BirthDay.Year);
            //}

            /////////////////////////////////////////////////

            //// 13) Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq
            /// Muellim, bu sherte uygun hec bir debtor olmadigina gore, men bir debtorun nomresini deyishdirmishem

            //List<Debtor> list = debtors
            //    .Where(deb =>
            //    {
            //        var l = deb.Phone.Split('-').ToList();
            //        string strPhone = "";
            //        foreach (var item in l)
            //            strPhone += item;
            //        for (int i = 0; i < strPhone.Length; i++)
            //            if (strPhone.Count(a => a == strPhone[i]) >= 2) 
            //                return false;
            //        return true;
            //    })
            //    .ToList();

            //Console.WriteLine("    Name             Phone       Debt\n");

            //foreach (var item in list)
            //{
            //    Console.Write("{0, -25}", item.FullName);
            //    Console.Write("{0, -20}", item.Phone);
            //    Console.WriteLine("{0, -20}", item.Debt);
            //}

            /////////////////////////////////////////////////

            //// 14) Tesevvur edek ki,butun borclari olanlar bugunden etibaren her ay 500 azn pul 
            ////     odeyecekler. Oz ad gunune kimi borcun oduyub qurtara bilenlerin siyahisin cixartmaq

            //List<Debtor> list = debtors
            //    .FindAll(deb => Math.Abs(deb.BirthDay.Month - DateTime.Now.Month) * 500 >= deb.Debt);
            //Console.WriteLine("       Name              Debt             Birthday\n");
            //foreach (var item in list)
            //{
            //    Console.Write("{0,-25}", item.FullName);
            //    Console.Write("{0,-15}", item.Debt);
            //    Console.WriteLine(" {0,-25}", item.BirthDay.ToShortDateString());
            //}
            ...
            //var list = debtors.Where(deb =>
            //{
            //    var fn = deb.FullName.ToLower();
